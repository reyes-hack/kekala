...
-- 1. Eliminar los indices que usan la columna month_year
DROP INDEX IF EXISTS public.idx_branch_fixed_costs_month;
DROP INDEX IF EXISTS public.idx_branch_fixed_costs_branch_month;

-- 2. Eliminar la columna month_year
ALTER TABLE public.branch_fixed_costs DROP COLUMN IF EXISTS month_year;
...
REVOKE ALL
ON FUNCTION public.get_income_statement_by_date(UUID, DATE, DATE)
FROM PUBLIC;
