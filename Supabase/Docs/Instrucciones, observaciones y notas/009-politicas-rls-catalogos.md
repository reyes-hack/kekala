# ASIGNACIÓN 009: Políticas RLS para Acceso de Lectura (Catálogos y Productos)

¡Hola Ángel!

Ahora que he conectado el Frontend, me he topado con que las tablas tienen RLS (Row Level Security) activado (¡lo cual está excelente por seguridad!). Sin embargo, como todavía no he implementado el módulo de Autenticación/Login, el frontend (que usa la `anon` key) recibía listas vacías (`[]`) al intentar leer las sucursales, los catálogos y los productos.

Para poder avanzar con mi desarrollo y permitir la lectura de los catálogos en esta fase, **he ejecutado directamente en la consola de Supabase el script de políticas RLS** para permitir `SELECT` público en algunas tablas y la inserción básica en `branch_inventory` (para la inicialización de stock).

Ya he guardado este script en el repositorio en la ruta correspondiente para mantener todo sincronizado.

## Cambios realizados en base de datos

He creado y guardado el script en: [002-public-read-policies.sql](KEKALA/Supabase/schema/rls/core/002-public-read-policies.sql)

Este es el contenido exacto que ejecuté:
...
> [!NOTE]
> Por favor, revisa que el archivo `.sql` esté en el orden correcto dentro de tus archivos de esquema y agrégalo a tu bitácora de seguimiento de base de datos. No necesitas ejecutar nada, ya está aplicado en producción/desarrollo.
