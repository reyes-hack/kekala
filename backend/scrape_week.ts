...
export async function runFoodbotScraper(dateInputArg?: string) {
  ...
  try {
    ...
    const formattedDateStart = "19/Aug/2026";
    const formattedDateEnd = "26/Aug/2026";
    const fullRangeStr = `${formattedDateStart} 00:00.00 - ${formattedDateEnd} 23:59.59`;
    ...
    const dateSet = await page.evaluate(() => {
      ...
      const startDate = new Date(2026, 7, 19, 0, 0, 0); // 19 Aug 2026
      const endDate = new Date(2026, 7, 26, 23, 59, 59); // 26 Aug 2026
...
runFoodbotScraper('2026-08-19').then(() => process.exit(0)).catch(e => { console.error(e); process.exit(1); });
