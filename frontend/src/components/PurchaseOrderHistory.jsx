          ...
          <div key={order.id} className="neo-surface fade-in" style={{ padding: '24px', borderRadius: '16px', display: 'flex', justifyContent: 'space-between', alignItems: 'center', borderLeft: `6px solid ${getStatusColor(order.status)}` }}>
            ...
            <div style={{ display: 'flex', gap: '16px', alignItems: 'center' }}>
              <div style={{ display: 'flex', flexDirection: 'column', gap: '4px' }}>
                ...
                <select 
                  className="neo-input"
                  style={{ padding: '8px', minWidth: '150px', background: 'var(--background-color)' }}
                  value={order.status}
                  onChange={(e) => handleStatusChange(order, e.target.value)}
                  disabled={order.status === 'ENTREGADA'}
                >
                  <option value="ENVIADA">ENVIADA</option>
                  <option value="ACEPTADA">ACEPTADA (Proveedor Confirmó)</option>
                  <option value="RECHAZADA">RECHAZADA</option>
                  <option value="ENTREGADA">ENTREGADA A SUCURSAL (Suma Inventario)</option>
                </select>
              </div>
